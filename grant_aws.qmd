...
The goals are to build an analytics data lake that centralizes broadband data to support broadband expansion, grant applications and research on broadband equity and development.
...
I think right now we are good with the AWS lambda function but we need a way to build intermediaries tools/workflow to allow us to get feedback from partners and build on top of it for better and more products. IE this data lake needs an efficient way to query, do quick and shareable visualization then it will be awesome if it integrates well on our "production" pipeline.
Then we need to build the infrastructure to ease the query of such data: do we have a spark question or duckDB/parquet question (scope needed here prob: R package verus use of AWS solution?).
