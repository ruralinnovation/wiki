::: callout-tip
*Note: DO NOT EDIT THIS README IN THE [`coriverse`](https://github.com/ruralinnovation/coriverse.git) REPO. It is derived from [`README.qmd`](https://github.com/ruralinnovation/wiki/edit/main/README.Rmd) in the [`wiki`](https://github.com/ruralinnovation/wiki.git) repo.*
:::
...
This package can be installed by using `devtools` or `remotes`, but a prerequisite for any of the work we do in `R` is a version control utility called [git](). Please make sure that the git command-line utility is installed on your system before proceeding.
...
-   [gfortran-12.2-universal.pkg](https://mac.r-project.org/tools/gfortran-12.2-universal.pkg){.external target="_blank"}
-   [Xcode](https://download.developer.apple.com/Developer_Tools/Command_Line_Tools_for_Xcode_15.1/Command_Line_Tools_for_Xcode_15.1.dmg){.external target="_blank"}

Once these prerequisites are installed, head over to [CRAN](//cran.r-project.org/bin/macosx/) and download the latest version of `R` for macOS 11 (and higher; `R` version [`4.4.2`](https:////cran.r-project.org/bin/macosx/big-sur-arm64/base){.external target="_blank"} as of the last update to these instructions).
...
``` r
    ...
    # ... the preferred way to do this is by editing your .Renvion file directly:
...
The following steps for adding a new function to `coriverse` (or any `R` package) are parts of "The Whole Game", the 1st chapter in the *Getting Started* section of [**R Packages**](https://r-pkgs.org/) from Hadley Wickham and Jenny Bryan and can be visualized here:
